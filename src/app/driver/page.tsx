...
export default function DriverPage() {
  // Broadcast Alert state (multi-alert)
  ...
  // Removed duplicate handleDismissBroadcast definition
  const [jobs, setJobs] = useState<ApiJob[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [drawerOpen, setDrawerOpen] = useState<boolean>(false);
  const [activeDrawerSection, setActiveDrawerSection] = useState<DrawerSection>("queue");
  const [panelExpanded, setPanelExpanded] = useState(false);
  const [pendingCompletionJob, setPendingCompletionJob] = useState<ApiJob | null>(null);
  ...
  const [incomingQueueDeadlineMs, setIncomingQueueDeadlineMs] = useState<number | null>(null);
  const [incomingCountdownSeconds, setIncomingCountdownSeconds] = useState(NEW_JOB_QUEUE_COUNTDOWN_SECONDS);
  const [soundAlertsEnabled, setSoundAlertsEnabled] = useState(true);
  const [externalMapsFallbackEnabled, setExternalMapsFallbackEnabled] = useState(true);
  ...
  const activeJob = useMemo(() => {
    ...
    return myJobs.find((job: ApiJob) => job.id === focusedJobId) ?? myJobs[0];
  }, [myJobs, focusedJobId]);

  const incomingJob = useMemo(() => {
    if (!incomingJobId) return null;
    return myJobs.find((job: ApiJob) => job.id === incomingJobId) ?? null;
  }, [myJobs, incomingJobId]);

  const queuedJobs = useMemo(() => {
    return myJobs.filter((job: ApiJob) => job.id !== activeJob?.id && job.id !== incomingJob?.id);
  ...
  useEffect(() => {
    ...
    if (hasActiveMission && !incomingJobId) {
      const newestUnseenJob = myJobs.find(
        (job) =>
          !seenJobIdsRef.current.has(job.id) &&
          job.id !== activeJob?.id
      );
  ...
  return (
    <>
        ...
        {showBroadcastAlert && broadcastAlerts.length > 0 && (
          <div className="fixed inset-x-0 top-24 z-50 flex justify-center">
            <div className="max-w-xs w-full rounded-xl border border-yellow-400/30 bg-yellow-900/90 px-3 py-2 shadow-md backdrop-blur flex items-center gap-2">
              <span className="inline-block rounded-full bg-yellow-400/20 px-2 py-0.5 text-[11px] font-semibold text-yellow-300">ALERT</span>
              {/* Show badge if multiple alerts */}
              {broadcastAlerts.length > 1 && (
                <span className="ml-1 rounded-full bg-yellow-400/30 px-2 py-0.5 text-[11px] font-semibold text-yellow-200">{broadcastAlerts.length} Alerts</span>
              )}
              {/* Truncate preview if long */}
              <span className="truncate text-xs font-medium text-yellow-100 flex-1 mx-2" style={{ maxWidth: '10em' }}>
            ...
            {alertPanelOpen && (
              <div className="absolute top-12 left-1/2 -translate-x-1/2 w-[320px] max-w-xs rounded-xl border border-yellow-400/30 bg-yellow-900/95 px-3 py-3 shadow-lg backdrop-blur z-50" style={{ maxHeight: '260px', overflowY: 'auto' }}>
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-xs font-semibold text-yellow-200">Broadcast Alerts</span>
                  <button
                    onClick={handleDismissBroadcast}
                ...
                <ul className="space-y-2">
                  {broadcastAlerts.map((alert, idx) => (
                    <li key={alert.timestamp} className="rounded-lg border border-yellow-400/20 bg-yellow-900/80 px-3 py-2 flex flex-col">
                      <span className="text-xs font-medium text-yellow-100 break-words" style={{ wordBreak: 'break-word' }}>{alert.message}</span>
                      <div className="mt-1 flex items-center justify-between">
                        <span className="text-[10px] text-yellow-200">{new Date(alert.timestamp).toLocaleTimeString()}</span>
                        <button
      ...
      {drawerOpen ? (
        <div className="mx-auto max-w-md shrink-0 px-4 pt-3">
          <div className="rounded-3xl border border-white/10 bg-slate-900/95 p-4">
            ...
            <p className="mt-1 text-sm text-white/60">{company?.name ?? "Dispatch Platform"}</p>
            ...
            {activeDrawerSection === "profile" ? (
              <div className="mt-4 space-y-3 text-sm">
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <p className="text-xs uppercase tracking-[0.2em] text-cyan-300">Current Mission</p>
                  <p className="mt-2 font-medium text-white">{activeJob?.name ?? "No active mission"}</p>
                  <p className="text-white/70">{activeJob?.service ?? "Waiting for dispatch"}</p>
                  <p className="text-white/60">{activeJob?.status ?? "Idle"}</p>
                </div>

                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <p className="text-xs uppercase tracking-[0.2em] text-cyan-300">Queued Jobs</p>
                  {queuedJobs.length > 0 ? (
                ...
                <div className="mt-4 space-y-3">
                  {queuedJobs.map((job) => (
                    <div key={job.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                      ...
                      <p className="mt-2 text-sm text-white/70">{job.service ?? "Service Request"}</p>
            ...
            {activeDrawerSection === "settings" ? (
              <div className="mt-4 space-y-3 text-sm">
                ...
                >
                  ...
                  <span className="text-cyan-200">{autoNavigateEnabled ? "On" : "Off"}</span>
                </button>
                ...
                >
                  ...
                  <span className="text-cyan-200">{soundAlertsEnabled ? "On" : "Off"}</span>
                </button>
                ...
                >
                  ...
                  <span className="text-cyan-200">{externalMapsFallbackEnabled ? "On" : "Off"}</span>
                </button>
              </div>
            ) : null}
          </div>
      ...
      <div className="mx-auto flex w-full max-w-md flex-1 min-h-0 px-4 pt-4 pb-0">
        <div className="relative h-full w-full overflow-hidden rounded-[28px] border border-cyan-500/20 bg-slate-900 shadow-[0_0_0_1px_rgba(34,211,238,0.08)]">
          ...
          <div className="h-full [&>*]:h-full [&_*]:!rounded-none [&_.leaflet-container]:!h-full">
            <DriverLeafletMap
              ...
              directionsActive={Boolean(
                activeJob && (isNavigationActive || autoNavigateEnabled || directionsOpen)
              )}
          ...
          {activeJob ? (
            <div className="absolute inset-x-0 bottom-0 z-20 rounded-t-[28px] border-t border-white/10 bg-slate-900/95 backdrop-blur-md">
              ...
              <div className="px-4 pb-4">
                  ...
                  <div className="mt-3 rounded-3xl bg-slate-700 p-4 text-white">
                    <div className="space-y-3 text-base">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span>
                          ETA: {activeJob.etaMinutes != null ? `${activeJob.etaMinutes} min` : "—"}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="mt-3 rounded-2xl border border-cyan-500/30 bg-cyan-500/10 px-3 py-2 text-sm text-cyan-100">
                  ...
                  <p className="text-cyan-200/90">Distance estimate: {distanceEstimateLabel}</p>
                </div>
                ...
                {directionsOpen ? (
                  <div className="mt-2 max-h-40 overflow-y-auto rounded-2xl border border-cyan-500/30 bg-slate-800/95 p-3 text-xs text-cyan-100 backdrop-blur">
                    ...
                    <p className="mt-1 font-medium">Route status: {activeJob.status}</p>
                    {routeData?.offRoute ? (
                    ...
                    <p className="mt-1 text-cyan-200/90">
                      Total distance: {routeData ? `${routeData.distanceMiles.toFixed(1)} mi` : distanceEstimateLabel}
                    </p>
                    <p className="text-cyan-200/90">
                      Estimated travel time: {routeData ? `${Math.max(1, Math.round(routeData.durationMinutes))} min` : "Estimating..."}
                    </p>
                    <p className="mt-1 font-medium">
                      Next step: {routeData?.nextInstruction ?? "Continue to destination"}
                    </p>

                    {routeData?.steps?.length ? (
                      <div className="mt-1 space-y-1">
                        {routeData.steps.slice(0, 3).map((step, index) => (
                          <p key={`${step.instruction}-${index}`} className="text-cyan-200/80">
            ...
            <div className="border-t border-white/10 bg-slate-950/95 px-4 py-4">
              <p className="text-sm uppercase tracking-[0.2em] text-cyan-300">Finding Jobs</p>
